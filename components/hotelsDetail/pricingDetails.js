  ...
  return (
    <Card style={styles.cardContainer}>
      ...
      <View style={styles.textContainer}>
        {props.hotelDetail.coupons.code === undefined ? (
          <Text>Apply Coupon</Text>
        ) : (
          <Text style={[styles.coupon, {marginTop: 10}]}>
            Coupon : {props.hotelDetail.coupons.code}
          </Text>
        )}
        <View style={styles.checkboxContainer}>
          <Text style={styles.checkboxText}>
            {props.hotelDetail.coupons.price === undefined
              ? ''
              : '₹' + props.hotelDetail.coupons.price}
          </Text>
          {props.data.data.coupons !== undefined ? (
            <CheckBox
              checked={
                props.hotelDetail.coupons.price !== undefined ? true : false
              }
              onChange={openModal}
            />
          ) : (
      ...
      {props.data.data.services !== undefined &&
        props.data.data.services.map(item => {
          ...
          return (
            <ExtraServices
              ...
              desc={''}
              quantity={item.service_type.search('qty') !== -1 ? true : false}
              price={'₹' + item.price}
            />
          );
        })}
      ...
      {props.data.data.price !== undefined && (
        <View>
          ...
          <View style={styles.textContainer}>
            ...
            <Text style={styles.total}>
              ₹{props.data.data.price.discount_after_price}
            </Text>
          </View>
        </View>
      )}
      <Modal
        ...
        <View>
          <TopNavSimple
            backHandler={() => setModal(!modal)}
            screenTitle="Apply Coupon"
          />
          <ScrollView
            style={styles.applyCoupon}
            showsVerticalScrollIndicator={false}>
            <Text style={[styles.heading, styles.couponHeading]}>
              Availabe Coupons
            </Text>
                    ...
                    <View style={styles.container} key={item.id}>
                      <View style={styles.coupons}>
                        <View style={styles.textContent}>
                          <Text style={styles.coupon}>{item.code}</Text>
                        </View>
                        <View style={styles.btnContent}>
                          <Button
                            ...
                            size="tiny"
                            onPress={() =>
                              closeModal(item.code, item.discount_price)
                            }>
                            Apply
                          </Button>
                        </View>
...
const mapDispatchToProps = dispatch => {
  return bindActionCreators(
    {LoadPrices: LoadPrices, addCoupons: addCoupons},
    dispatch,
  );
};
