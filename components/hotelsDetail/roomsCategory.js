        ...
        <View style={styles.amenitiesContainer}>
          {props.amenitiesData.map(item => (
            <View key={item.id} style={styles.amenities}>
              <Image style={styles.amenitiesImg} source={{uri: item.image}} />
            </View>
          ))}
        </View>
      </ScrollView>
      ...
      {props.data.map(item => {
        ...
        return (
          <View key={item.id} style={styles.controlContainer}>
                ...
                <View style={styles.roomAmenities}>
                  {item.amenities.slice(0, 4).map(amenity => (
                    <Image
                      ...
                      style={styles.roomAmenitiesImg}
                    />
                  ))}
                  {item.amenities.length > 4 ? (
                    <Ripple
                      style={styles.moreBorder}
                      onPress={() => toggleModal(item.amenities)}>
            ...
            <ImageViewer
              images={modalImages}
              show={props.hotelDetail.showImageViewer}
