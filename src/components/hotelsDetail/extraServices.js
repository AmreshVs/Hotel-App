...
// console.log(props.hotelDetail);
  const checkChange = async (id, service_id) => {
    ...
    if (!checkArr.includes(props.id)) {
      ...
      setTimeout(function () {
        props.LoadPrices({ hotelId: props.hotelDetail.hotelIds.hotelId, roomId: props.hotelDetail.hotelIds.roomId, dates: props.hotelDetail.dates, rooms: props.hotelDetail.rooms, service: serviceArr, foods: props.hotelDetail.foods, coupons: props.hotelDetail.coupons.code || '' }, props.common.userData.access_token);
      }, 10);
    }
    ...
    if (type === 'add' && checkArr[props.id].qty > 0) {
      ...
      setTimeout(function () {
        if (props.hotelDetail.serviceChecked.includes(props.id)) {
          props.LoadPrices({ hotelId: props.hotelDetail.hotelIds.hotelId, roomId: props.hotelDetail.hotelIds.roomId, dates: props.hotelDetail.dates, rooms: props.hotelDetail.rooms, service: props.hotelDetail.services, foods: props.hotelDetail.foods, coupons: props.hotelDetail.coupons.code || '' }, props.common.userData.access_token);
        }
      }, 5);
    }
